...
app.get('/users/:u_id', async (req, res) => {
  ...
  query(qs1).then(data => {
      ...
      let qs2 = //to get all the posts the user has posted
...
//route to get a specific posts and their comments
app.get('/posts/:p_id', async (req, res) => {
  try {
    ...
    query(qs1).then(data => {
      ...
      let qs2 = //to get all the comments a post has
...
app.post('/users', async (req, res) => {
  try {
    ...
    const hashedPassword = await bcrypt.hash(password, saltRounds); //encrypts the password to be stored in the database
    ...
    query(qs1).then(data => {
      ...
      let user_id_qs = `SELECT COALESCE(MAX(u_id), 0) AS max_user_id FROM users`; //is used to determine a new user id to make
...
//route allows a user to login confirming their username and password
...
app.post('/users/:u_id/posts', async (req, res) => {
  try {
    ...
    query(qs1).then(data => {
      ...
      let post_id_qs = `SELECT COALESCE(MAX(p_id), 0) AS max_post_id FROM posts`; //is used to determine a new post id to make
...
//route creates a new comment on a post
  ...
  try {
    ...
    query(qs1).then(data => {
      ...
      query(qs2).then(data => {
        ...
        let comment_id_qs = `SELECT COALESCE(MAX(c_id), 0) AS max_comment_id FROM comments`; //is used to determine a new comment id to make
...
//route deletes a post and all the comments tied to it
      ...
      query(qs2).then(data => {
        ...
        query(qs3).then(data => {
          if (data.rows.length > 0) { 
            ...
            return query(qs4).then(() => res.json({ message: "Post and comments deleted successfully" }));
          }
...
//route deletes comment from a post
...
//route deletes a user and the posts and comments tied to them
...
//route adds a dislike from a post
...
//route removes a like from a post
...
//route removes a dislike from a post
...
//route adds a like from a comment
...
//route adds a dislike from a comment
...
//route removes a like from a comment
...
//route removes a dislike from a comment
